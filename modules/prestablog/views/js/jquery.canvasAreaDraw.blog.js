...
(function( $ ){
  ...
  var init = function(index, input, options) {
    ...
    ratiol = $(input).data("ratiol");
    ratioh = $(input).data("ratiol");
    ...
    $reset = $('<button type="button" class="btn"><i class="icon-trash"></i> '+$(input).data("lclear")+'</button>');
    $canvas = $('<canvas>');
    ...
    $canvas.addClass( "map" );
    $canvas.attr('usemap', $(input).data("usemap"));
    ...
    record = function() {
      if (points.length < 2) {
        $("#blocklinktolookbook").fadeOut();
      }
      else {
        $("#blocklinktolookbook").fadeIn();
      }
      ...
      for (var i = 0; i < points.length; i+=2) {
        pointsResize.splice(i, 0, Math.round(points[i] * ratiol), Math.round(points[i+1] * ratioh));
      }

      $("#lookbook_shape").val(pointsResize.join(','));
    };
