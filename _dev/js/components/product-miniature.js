  ...
  init(){
    $('.js-product-miniature').each((index, element) => {
      ...
      if($(element).find('.color').length > 5){
        ...
        $(element).find('.js-count-color').append(`+${count}`);
      }

      if($(element).find('.taille').length > 5){
        let count = 0;
        $(element).find('.taille').each((index, element) =>{
          if(index > 4){
            $(element).hide();
            count ++;
          }
        });
        $(element).find('.js-count-taille').append(`+${count}`);
      }
